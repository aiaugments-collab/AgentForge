...
By default, navigating within your embedded instance of AgentForge doesn't affect the client's browser history or viewed URL. AgentForge only provide a **handler**, that trigger on every route change in the **iframe**.

## Automatically Sync URL 

You can use the following snippet when configuring the SDK, which will implement a handler that syncs the AgentForge iframe with your browser:
...
window.addEventListener("popstate", () => {
  const route = activepieces.extractAgentForgeRouteFromUrl({ vendorUrl: window.location.href });
