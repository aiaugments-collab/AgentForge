...
This documentation explains how to embed the AgentForge iframe inside your application and customize it.
...
| --- | --- | --- | --- |
| instanceUrl| ✅ |  string | The url of the instance hosting AgentForge, could be https://cloud.activepieces.com if you are a cloud user. |
| jwtToken | ✅  |  string | The jwt token you generated to authenticate your users to AgentForge. |
| prefix | ❌ | string | Some customers have an embedding prefix, like this `<embedding_url_prefix>/<AgentForge_url>`. For example if the prefix is `/automation` and the AgentForge url is `/flows` the full url would be `/automation/flows`. |
| embedding.containerId | ❌ | string | The html element's id that is going to be containing AgentForge's iframe. |
...
| navigation.handler  | ❌ | `({route:string}) => void`  | This callback will be triggered each time a route in AgentForge changes, you can read more about it [here](/embedding/navigation) |
<Tip>
