...
This page focuses on describing the main components of AgentForge and focus mainly on workflow executions.
...
**Third Party**:
- **Postgres**: The main database for AgentForge.
...
- **Webhooks**:  
  All webhooks are sent to the AgentForge app, which performs basic validation and adds them to the queue. In case of a spike, webhooks will be added to the queue.
...
To scale AgentForge, you typically need to increase the replicas of either workers, the app, or the Postgres database. A small Redis instance is sufficient as it can handle thousands of jobs per second and rarely acts as a bottleneck.
