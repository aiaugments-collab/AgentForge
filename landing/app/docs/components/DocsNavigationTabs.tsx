...
export default function DocsNavigationTabs() {
  ...
  return (
    <div className="sticky top-16 z-20 bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm border-b border-gray-200 dark:border-gray-700 hidden lg:flex px-12 h-12">
      <div className="nav-tabs h-full flex text-lg gap-x-8">
        {navigationTabs.map((tab) => {
          ...
          return (
            <Link
              key={tab.name}
              href={tab.href}
              className={`link nav-tabs-item group relative h-full gap-2 flex items-center hover:text-gray-800 dark:hover:text-gray-300 focus:outline-primary dark:focus:outline-primary-light ${
                isActive
