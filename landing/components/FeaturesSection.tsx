...
export default function FeaturesSection() {
  ...
  return (
    <section className="bg-white dark:bg-gray-900 my-16">
      <div className="mx-10 max-w-screen-xl px-6 sm:py-6 max-[767px]:px-0">
        <div className="max-w-screen-md mb-2 lg:mb-16">
          <h2 className="mb-4 text-[2rem] md:text-[2.75rem] text-center md:text-left tracking-tight font-bold text-gray-900 dark:text-white">
            <span>Augment enterprise-ready Agents</span>
          </h2>
        </div>
        
        <div className="gap-8 items-center xl:gap-16 flex flex-col md:flex-row" id="features-tab">
          {/* Feature Categories and Tabs */}
          <div className="mt-4 md:mt-0 basis-2/5 px-4 md:px-0 self-stretch flex-nowrap gap-4 md:space-y-10 md:gap-0 md:flex-wrap md:whitespace-normal overflow-scroll md:overflow-auto flex md:flex-col">
            {categories.map((category) => (
              <div key={category} className="inline-block md:block">
                <h3 className="text-xl font-bold mb-3.5 hidden md:block">{category}</h3>
                <div className="flex flex-row flex-nowrap md:flex-wrap gap-4" role="tablist">
                  {getFeaturesByCategory(category).map((feature) => (
                    <button
                      ...
                      className={`group relative outline-none focus:ring-0 flex whitespace-nowrap items-center py-2.5 px-5 text-lg font-medium focus:outline-none rounded-lg border transition-all duration-300 ${
          ...
          {/* Feature Content */}
          <div className={`basis-3/5 py-8 md:py-14 px-8 md:px-12 md:rounded-l-[3rem] ${activeFeatureData.backgroundColor}`}>
            <h2 className="pr-14 text-[1.75rem] md:text-[2.75rem] tracking-tight font-medium text-gray-900 dark:text-white mb-6">
